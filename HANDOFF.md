...
- **Publicação candidata:** O workflow foi ajustado para validar PR sem deploy; no push da `main`, verifica serviço e disco, cria snapshot local de `data/users.json`, constrói a imagem antes de trocar o container, faz healthcheck e tenta rollback para a imagem anterior em falha. Remove `chmod 777`, `compose down` antes do build, `git reset --hard` na VPS e modificação automática do Caddy. O script aborta sem trocar o container caso `users.json` ou configuração básica estejam ausentes.
- **Riscos/pendências:** Senhas legadas em texto puro permanecem no arquivo até serem redefinidas; nenhuma migração automática modifica a base no startup. O arquivo `users.json` continua sendo a persistência do cadastro de usuários neste fluxo; não houve migração para Appwrite nesta alteração. O snapshot no workflow é local à VPS; a existência de backup externo, capacidade real do host e funcionamento do rollback em produção ainda não foram verificados. Próximo passo: integrar a branch sem sobrescrever as alterações do Gemini, revisar o diff final e submeter para homologação.
